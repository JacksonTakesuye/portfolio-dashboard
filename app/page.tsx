...
export default function Home() {
  ...
  useEffect(()=>{
    async function loadData(){
      ...
      if(e1)setError(e1.message)
        ...
        const pm:Record<string,any>={};(psr||[]).forEach((p:any)=>{if(!pm[p.property_id])pm[p.property_id]=p});setPsrReports(pm)
      }
      setLoading(false)
    }
    loadData()
  ...
  const savePsr=async()=>{
    ...
    else{showToast('PSR saved');setPsrReports((prev:any)=>({...prev,[detailProp.id]:{...psrForm,property_id:detailProp.id}}))}
    setSavingPsr(false)
  }
