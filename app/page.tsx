        ...
        {detailProp&&(
          <div style={{width:'400px',background:'#fff',borderLeft:'1px solid #e2e8f0',display:'flex',flexDirection:'column',flexShrink:0,height:'calc(100vh - 140px)',position:'sticky',top:'0',overflowY:'auto'}}>
            ...
            <div style={{padding:'16px 20px',flex:1,overflowY:'auto'}}>
              ...
              {detailTab==='documents'&&(<div><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'12px'}}><div style={{fontWeight:'700',fontSize:'13px',color:'#1e293b'}}>Documents</div><label style={{padding:'5px 10px',background:'#3b82f6',color:'#fff',borderRadius:'6px',fontSize:'11px',fontWeight:'600',cursor:'pointer'}}>+ Upload PDF<input type='file' accept='application/pdf' style={{display:'none'}} onChange={e=>{const file=e.target.files?.[0];if(file)uploadDocument(file)}}/></label></div>{propDocuments.length===0?<div style={{background:'#f8fafc',borderRadius:'8px',padding:'24px',textAlign:'center',border:'2px dashed #e2e8f0'}}><div style={{fontSize:'24px',marginBottom:'8px'}}>PDF</div><div style={{fontWeight:'600',fontSize:'13px',color:'#334155',marginBottom:'4px'}}>No documents yet</div><div style={{fontSize:'12px',color:'#94a3b8'}}>Upload PDFs using the button above.</div></div>:propDocuments.map((doc:any,i:number)=>(<div key={i} style={{border:'1px solid #e2e8f0',borderRadius:'8px',padding:'12px',marginBottom:'8px',background:'#fafafa',display:'flex',justifyContent:'space-between',alignItems:'center'}}><div><div style={{fontWeight:'600',fontSize:'12px',color:'#1e293b'}}>{doc.file_name}</div><div style={{fontSize:'11px',color:'#94a3b8',marginTop:'2px'}}>{doc.created_at?.split('T')[0]} {doc.file_size?Math.round(doc.file_size/1024)+'KB':''}</div></div><button onClick={()=>viewDocument(doc.file_path)} style={{padding:'5px 10px',background:'#eff6ff',color:'#1d4ed8',border:'none',borderRadius:'6px',fontSize:'11px',fontWeight:'600',cursor:'pointer'}}>View</button></div>))}</div>)}
            </div>
          </div>
        )}
      </div>
